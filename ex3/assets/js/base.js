  ...
  pokemonsTable = $("#pokemons-table").DataTable({
    ...
    columnDefs: [
      ...
      },
      {
        targets: 1,
        render: function (data, type, row, meta) {
          return row.types.map((type) => `<span class="badge ${getTypeClass(type)}">${capitalizeFirstLetter(type)}</span>`).join(" ");
        }
      },
      {
        targets: 2,
...
function updatePokemonDetails(index) {
  var pokemon = pokemonList[index];
  var pokemonBackgroundElement = document.getElementById("pokemon-background");
  var pokemonNameElement = document.getElementById("pokemon-name");
  var pokemonImageElement = document.getElementById("pokemon-image");
  var pokemonTypesElement = document.getElementById("pokemon-types");
  var pokemonStatsHP = document.getElementById("pokemon-hp");
  var pokemonStatsAttack = document.getElementById("pokemon-attack");
  var pokemonStatsDefense = document.getElementById("pokemon-defense");
  var pokemonStatsSpecialAttack = document.getElementById("pokemon-special-attack");
  var pokemonStatsSpecialDefense = document.getElementById("pokemon-special-defense");
  var pokemonStatsSpeed = document.getElementById("pokemon-speed");

  pokemonBackgroundElement.className = getTypeClass(pokemon.types[0]);
  pokemonNameElement.innerHTML = capitalizeFirstLetter(pokemon.name);
  pokemonImageElement.src = pokemon.image;
  pokemonTypesElement.innerHTML = pokemon.types.map((type) => `<span class="badge ${getTypeClass(type)}">${capitalizeFirstLetter(type)}</span>`).join(" ");
  pokemonStatsHP.innerHTML = pokemon.stats[0].base_stat;
  pokemonStatsAttack.innerHTML = pokemon.stats[1].base_stat;
  pokemonStatsDefense.innerHTML = pokemon.stats[2].base_stat;
  pokemonStatsSpecialAttack.innerHTML = pokemon.stats[3].base_stat;
  pokemonStatsSpecialDefense.innerHTML = pokemon.stats[4].base_stat;
  pokemonStatsSpeed.innerHTML = pokemon.stats[5].base_stat;
}
